...
# Agora você pode seguir com o restante do código como antes, por exemplo, calculando as porcentagens de sentimentos por banco
...
df_short.to_csv('app_reviews_new.csv', index=False)
df_short_new = pd.read_csv('app_reviews_new.csv')
...
start_date, end_date = st.date_input("Selecione o intervalo de datas:", [df_short_new['at'].min(), df_short_new['at'].max()])

# Filtra o DataFrame de acordo com o intervalo de datas selecionado
df_filtered = df_short_new[(df_short_new['at'] >= pd.to_datetime(start_date)) & (df_short_new['at'] <= pd.to_datetime(end_date))]
...
# Calcula as porcentagens de sentimentos por banco
porcentagens = calcular_porcentagens(sentimentos_por_banco)

# Ranking dos bancos com base na porcentagem de sentimentos positivos
ranking = ranking_bancos(porcentagens)

st.subheader("Ranking dos Bancos")
